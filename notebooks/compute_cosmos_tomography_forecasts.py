#!/usr/bin/env python
"""Compute the three- and six-bin full-COSMOS LLLELP forecasts."""
...
FORECASTS = {
    'cosmos-3bin': {
        ...
        'output': (
            ROOT / 'results' / 'fisher_results_cosmos_3bin_lllelp.pkl'
        ),
    },
    'cosmos-6bin': {
        'data_dir': (
        ...
        'output': (
            ROOT / 'results' / 'fisher_results_cosmos_6bin_lllelp.pkl'
        ),
    },
}
...
for name, paths in FORECASTS.items():
    print('\n' + '=' * 70)
    print(f'Computing {name} LLLELP Omega_m/sigma_8 forecast')
    print('=' * 70)
    ...
    forecast.setup(param_names=['Omega_m', 'sigma_8'])
