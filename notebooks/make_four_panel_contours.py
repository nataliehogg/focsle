...
for ax, (title, pkl) in zip(axes.ravel(), PANELS):
    results = FisherForecast.load_results(pkl)
    ...
    # The forecast ellipses are (much) smaller than a pixel at DES scale:
    # mark the fiducial on the main panel and show the ellipse shape in a
    # zoomed inset with its own limits (fiducial +/- 4 sigma per axis).
    # ax.plot(*fiducial, marker='+', color=warm[2], ms=8, mew=1.5, zorder=101)

    # C = np.linalg.inv(F_combined)
    # sig_om, sig_s8 = np.sqrt(np.diag(C))

    # axins = ax.inset_axes([0.06, 0.06, 0.38, 0.38])
    # plot_fisher_ellipse(F_combined, fiducial, axins,
    #                     color=warm[2], show_2sigma=True)
    # axins.set_xlim(fiducial[0] - 4 * sig_om, fiducial[0] + 4 * sig_om)
    # axins.set_ylim(fiducial[1] - 4 * sig_s8, fiducial[1] + 4 * sig_s8)
    # axins.set_xticks([])
    # axins.set_yticks([])
    # axins.set_box_aspect(1)
    # axins.text(0.5, -0.16,
    #            rf'$\sigma(\Omega_{{\rm m}})={sig_om:.1e},\ '
    #            rf'\sigma(\sigma_8)={sig_s8:.1e}$',
    #            transform=axins.transAxes, ha='center', va='top', fontsize=6)

    fom = np.sqrt(np.linalg.det(F_combined))
    print(f'{title}: FoM = {fom:.3e} ({fom / fom_des:.0f}x DES)')
...
legend_handles = [
    mpatches.Patch(color=cool[1], alpha=0.6, label=r'DES Y3 EEEPPP'),
    mpatches.Patch(color=warm[2], alpha=0.6, label=r'Euclid LOS forecast'),
]
